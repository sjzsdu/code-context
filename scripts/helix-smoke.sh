...
answer_context_cli="$(run_code_context answer "Where is HealthMessage used?" --context-only --json --template explain --limit 5 --target-kind symbol --text-weight 0.6 --vector-weight 0.4)"
printf '%s\n' "$answer_context_cli"
...
answer_cli="$(run_code_context answer "Where is HealthMessage used?" --template plan --system-prompt "Answer from smoke evidence and cite sources." --json --limit 5)"
printf '%s\n' "$answer_cli"
...
answer_api="$(curl -fsS -X POST "http://127.0.0.1:${HTTP_PORT}/api/answer" -H 'Content-Type: application/json' --data '{"question":"Where is HealthMessage used?","template":"review","system_prompt":"Answer from smoke evidence and cite sources.","filter":{"target_kinds":["symbol"]},"text_weight":0.6,"vector_weight":0.4,"limit":5}')"
printf '%s\n' "$answer_api"
