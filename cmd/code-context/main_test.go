...
func TestConfigInspectCmdPrintsMergedConfigAndSources(t *testing.T) {
		...
		Config struct {
			...
			Store struct {
				...
				Helix   struct {
					...
					ReadRetryAttempts  int           `json:"read_retry_attempts"`
					ReadRetryBackoff   time.Duration `json:"read_retry_backoff"`
	...
	if decoded.Config.Store.Helix.ReadRetryAttempts != 3 {
		t.Fatalf("store.helix.read_retry_attempts = %d", decoded.Config.Store.Helix.ReadRetryAttempts)
	}
	if decoded.Config.Store.Helix.ReadRetryBackoff != 40*time.Millisecond {
		t.Fatalf("store.helix.read_retry_backoff = %s", decoded.Config.Store.Helix.ReadRetryBackoff)
	}
