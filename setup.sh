...
echo "构建完成: $(ls -lh code-context-mcp | awk '{print $5}') binary"
echo ""

# 安装
mkdir -p "$HOME/.local/bin"
mv code-context "$HOME/.local/bin/"
chmod +x "$HOME/.local/bin/code-context"
mv code-context-mcp "$HOME/.local/bin/"
chmod +x "$HOME/.local/bin/code-context-mcp"
...
ls -lh "$HOME/.local/bin/code-context-mcp"
echo ""
echo "使用方法:"
...
echo "  code-context snapshot               # 生成项目级 LLM 上下文"
echo "  code-context snapshot \"auth\"        # 生成主题相关 LLM 上下文"
echo "  code-context stats                 # 查看统计"
echo "  code-context serve                 # 启动 HTTP 服务"
echo ""
echo "  code-context-mcp --root .          # 启动 MCP 服务器"
echo ""
echo "MCP 配置示例 (Claude Desktop):"
echo "  {"
echo "    \"mcpServers\": {"
echo "      \"code-context\": {"
echo "        \"command\": \"$HOME/.local/bin/code-context-mcp\","
echo "        \"args\": [\"--root\", \".\"]"
echo "      }"
echo "    }"
echo "  }"
